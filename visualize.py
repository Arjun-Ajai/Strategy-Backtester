...
def plot_results(df_ohlcv, equity, trades, run_id, symbol):
    ...
    ax1.legend()
    buy_dates=[trades.date for t in trades if trades.action == 'BUY']
    buy_prices=[trades.price for t in trades if trades.action == 'BUY']
    sell_dates=[trades.date for t in trades if trades.action == 'SELL']
    sell_prices=[trades.price for t in trades if trades.action == 'SELL']
    ...
    equity_dates=[equity.date for e in equity ]
    equity_values=[equity.portfolio_value for e in equity]
